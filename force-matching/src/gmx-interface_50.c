...
/**
@file gmx-interface_50.c 
@authors Will Noid, Wayne Mullinax, Joseph Rudzinski, Nicholas Dunn, Michael DeLyser
@brief Functions to interface cgff and cgmap with the gmx library version 5.0
*/
...
int get_natoms(tW_gmx_topology *self)
{
    ...
    return self->contents->atoms.nr; // THIS IS THE SAME
}
...
char *get_name(tW_gmx_topology *self)
{
    ...
    return *(self->contents->name); // THIS IS THE SAME
}
...
int get_nbonds(tW_gmx_topology *self){
   return self->contents->idef.il[F_BONDS].nr; // THIS IS THE SAME
}
...
int get_nangles(tW_gmx_topology *self){
   return self->contents->idef.il[F_ANGLES].nr; // THIS IS THE SAME
}
...
int get_ndihs(tW_gmx_topology *self){
   return self->contents->idef.il[F_RBDIHS].nr; // THIS IS THE SAME
}
...
int get_npairs(tW_gmx_topology *self){
   return self->contents->idef.il[F_LJ14].nr; // THIS IS THE SAME
}
...
int *get_bond_list(tW_gmx_topology *self){
   ...
   return bond_list; // THIS IS THE SAME
}
...
int *get_angle_list(tW_gmx_topology *self){
   ...
   return angle_list; // THIS IS THE SAME
}
...
int *get_dih_list(tW_gmx_topology *self){
   ...
   return dih_list; // THIS IS THE SAME
}
...
int *get_pair_list(tW_gmx_topology *self){
   ...
   return pair_list; // THIS IS THE SAME
}
...
int get_resind(tW_gmx_topology *self, int i_atom)
{
    ...
    return self->contents->atoms.atom[i_atom].resind; // THIS IS THE SAME
}
...
char *get_atomtype(tW_gmx_topology *self, int i_atom)
{
    ...
    return *(self->contents->atoms.atomtype[i_atom]); // THIS IS THE SAME
}
...
int get_nexcl(tW_gmx_topology *self, int i_atom)
{
    ...
    return nr_excl; // THIS IS THE SAME
}
...
int *get_excl_list(tW_gmx_topology *self, int i_atom)
{
    ...
    return excl_list; // THIS IS THE SAME
}
...
void read_tpr(tW_gmx_topology *top, const char *tpr_fnm)
{
    ...
    top->b_tpr = read_tps_conf(tpr_fnm, title, top->contents, &ePBC, NULL, NULL, box, FALSE); // THIS IS THE SAME
...
bool get_top(tW_word coord_fnm, tW_gmx_topology *top, bool TPR_flag, tW_word tpr_filename)
{
    ...
    return top->b_tpr; // THIS IS THE SAME ?
}
...
int print_tpr_file(FILE * fp_log, tW_gmx_topology *top)
{
    ...
    return 0; // THIS SHOULD BE THE SAME
}
...
;;;;
	...
	output_env_init_default((struct output_env **)frame->oenv);

//	frame->oenv.time_unit = time_ps;
//	frame->oenv.view = FALSE;
//	frame->oenv.xvg_format = exvgNONE;
//	frame->oenv.verbosity = 0;
...
int read_first_trxframe(tW_gmx_trxframe *frame, const char *trx_fnm)
{
    ...
    frame->contents = emalloc(sizeof(t_trxframe));
    int answer = read_first_frame((output_env_t)frame->oenv, &frame->status, trx_fnm, frame->contents, in_flags);
...
{
    return read_next_frame((output_env_t) frame->oenv, frame->status, frame->contents);
...
/* Starting in GROMACS 5.0, the -h command line option is caught BEFORE parse_common_args is called.
 * So, if we pass in the -h option to parse_common_args, the program terminates and says that -h is
 * an invalid command line option. So, before we call parse_common_args, I'm checking for -h, and if
 * I find it, I'm displaying the table that we got when passing -h to cgmap when compiled with 
 * GROMACS 4.5.3 */
    for (i=0;i<argc;i++)
    {
	if (strcmp(argv[i],"-h") == 0)
	{
            ...
            fprintf(stderr,"------------------------------------------------------------\n");
            fprintf(stderr,"  -p        map.top  Input        Topology file\n");
            fprintf(stderr,"  -f         aa.xtc  Input        Trajectory: xtc trr trj gro g96 pdb cpt\n");
            fprintf(stderr,"  -o         cg.xtc  Output       Trajectory: xtc trr trj gro g96 pdb cpt\n");
            fprintf(stderr,"  -s         cg.tpr  Input, Opt.  Portable xdr run input file\n");
            fprintf(stderr,"  -c         cg.gro  Output, Opt. Coordinate file in Gromos-87 format\n");
            fprintf(stderr,"\n");
            fprintf(stderr,"Option       Type   Value   Description\n");
            fprintf(stderr,"------------------------------------------------------\n");
            fprintf(stderr,"-[no]h       bool   yes     Print help info and quit\n");
            fprintf(stderr,"-[no]version bool   no      Print version info and quit\n");
            fprintf(stderr,"-nice        int    0       Set the nicelevel\n");
            fprintf(stderr,"-b           time   0       First frame (ps) to read from trajectory\n");
            fprintf(stderr,"-e           time   0       Last frame (ps) to read from trajectory\n");
            fprintf(stderr,"-dt          time   0       Only use frame when t MOD dt = first time (ps)\n");
            fprintf(stderr,"-[no]w       bool   no      View output xvg, xpm, eps and pdb files\n");
	    exit(0);
	}
    }
