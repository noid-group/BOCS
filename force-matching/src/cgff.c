        ...
        {                /* JFR - added 04.06.12: If in SECOND_HALF MODE, skip the trr loop */
          ...
          do
          {
            ...
            if ((sys.REF_var.flag_splitfiles == TRUE)|| (n_frames % np == local_rank))
            {
              ...
              {
                fprintf(stderr,"rank % 3d fr % 6d file %d\n",local_rank,n_frames,i+1);
              }
          ...
          fprintf (files.fp_log, "Used %d frames out of %d frames read for %s.\n\n", n_frames_local, n_frames, files.structures[i]);
