...
FixNH::FixNH(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  while (iarg < narg) {
    ...
    // ************************************************************************************************
    // NJD 
    }  else if (strcmp(arg[iarg],"cgiso") == 0) { //NJD - added 8/19/13
        if (iarg+4 > narg) error->all(FLERR,"Illegal fix nvt/npt/nph command");
	...
	{
		if (iarg+4 > narg) error->all(FLERR,"Illegal CG fix nvt/npt/nph command");
		...
		if (iarg+N_p_match > narg) error->all(FLERR,"Illegal CG fix nvt/npt/nph command");
	...
	{
		 if (iarg+3 > narg) error->all(FLERR,"Illegal CG fix nvt/npt/nph command");

		p_basis_type = 1;
	
		splines = read_F_table( arg[iarg+1] );
	...
	} else if (strcmp(arg[iarg], "cubic_spline") == 0 )
	{
		...
		splines = build_cubic_splines(splines);

		iarg += 3;
	}  else 
	{
		fprintf(stderr, "ERROR: CG BASIS TYPE %s NOT RECOGNIZED\n", arg[iarg+4]);
		exit(1);
	}
...
void FixNH::init()
{
  ...
  if (pstat_flag) {
    ...
    if (p_match_flag){    // NJD 10/17/13
      if ( p_basis_type == 0 )
      {
        pressure->send_cg_info(p_basis_type, N_p_match, p_match_coeffs, N_mol, vavg);
      } else if ( p_basis_type == 1 || p_basis_type == 2  )
      {
	pressure->send_cg_info(p_basis_type, splines);
      }
    }
  }
...
/* ------------------------------------------------------------------------
   read in F(V) from csv file - NJD 12/17/13
--------------------------------------------------------------------------*/
std::vector< std::vector<double> > FixNH::read_F_table( char *filename )
{
	char separator = ',';
	...
	if ( input_file.is_open() )
	{
		...
		while( std::getline(input_file,line) )
		{
			...
			while( std::getline( ss, token, separator )  )
			{
				...
				token = token.substr(strBegin, strRange);

				double dub =  atof(token.c_str() );
				input_data[col].push_back(dub);
...
/* ------------------------------------------------------------------------
   builds the array of cubic splines from the linear table - NJD 12/18/13
--------------------------------------------------------------------------*/
std::vector< std::vector<double> > FixNH::build_cubic_splines(std::vector< std::vector<double> >& data )
{
	//variable names come from "Spline (mathematics)" article on wikipedia where the algorithm is defined
	std::vector<double> a, b, d, h, alpha, c, l, mu, z;
	...
	for (int i=0; i<n; i++)
	{
		...
		if (i>1 && i<(n-1))
		{
			alpha_i = (3.0 / h[i]) * ( data[1][i+1] - data[1][i]) - (3.0 / h[i-1] ) * ( data[1][i] - data[1][i-1] );
	...
	for (int i=1; i<n-1; i++)
	{
		l.push_back( 2*( data[0][i+1] - data[0][i-1] ) - h[i-1]*mu[i-1] );
	...
	for (int i=0; i<n-1; i++)
	{
		splines[1].push_back(a[i]);
		splines[2].push_back(b[i]);
		splines[3].push_back(c[i]);
		splines[4].push_back(d[i]);
		splines[0].push_back(data[0][i]);
	}
...
void FixNH::couple()
{
  ...
  if (pstyle == ISO){
    ...
    fprintf(stderr, "vector in couple\n");
  } else if (pcouple == XY) {
