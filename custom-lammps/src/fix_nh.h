...
class FixNH : public Fix {
 public:
  ...
  std::vector< std::vector<double> > read_F_table(char* filename); // NJD 12-17-13
  std::vector< std::vector<double> > build_cubic_splines(std::vector< std::vector<double> >& linear_table ); // NJD 12-18-13
 ...
 protected:
  ...
  int p_basis_type; 		   // NJD 12/17/13 - type of basis function to use
  ...
  int N_p_match;                   // NJD 8/19/13 - the number of basis functions to use in p_matching
  double *p_match_coeffs;          // NJD 8/19/13 - an array of the coefficients phi
